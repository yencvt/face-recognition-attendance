...
void CameraPlugin::RegisterWithRegistrar(
  ...
  plugin->method_channel_ =
    std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
      registrar->messenger(), "flutter_cam/camera_windows_ext",
...
void CameraPlugin::HandleMethodCall(
  ...
  if (method_call.method_name() != "getLatestFrameBgra") {
  ...
  const auto id_it = args.find(EncodableValue("cameraId"));
  if (id_it == args.end()) {
    result->Error("bad_args", "Missing cameraId");
    return;
  }

  int64_t camera_id = -1;
  if (std::holds_alternative<int32_t>(id_it->second)) {
    camera_id = std::get<int32_t>(id_it->second);
  } else if (std::holds_alternative<int64_t>(id_it->second)) {
    camera_id = std::get<int64_t>(id_it->second);
  } else {
    result->Error("bad_args", "cameraId must be int");
    return;
  }

  auto camera = GetCameraByCameraId(camera_id);
  if (!camera) {
    result->Error("camera_error", "Camera not found");
    return;
  }

  auto cc = camera->GetCaptureController();
  if (!cc) {
    result->Error("camera_error", "Capture controller unavailable");
    return;
  }
  ...
  uint32_t width = 0;
  uint32_t height = 0;
  if (!cc->GetLatestFrameBgra(&bytes, &width, &height) || bytes.empty()) {
    result->Success(EncodableValue());
    return;
  }

  EncodableMap payload;
  payload[EncodableValue("width")] = EncodableValue(static_cast<int64_t>(width));
  payload[EncodableValue("height")] = EncodableValue(static_cast<int64_t>(height));
  payload[EncodableValue("bytes")] = EncodableValue(bytes);
