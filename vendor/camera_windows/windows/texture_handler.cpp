...
bool TextureHandler::GetLatestFrameBgra(std::vector<uint8_t>* out,
                                        uint32_t* width,
                                        uint32_t* height) {
  if (!out || !width || !height) {
    return false;
  ...
  const uint32_t frame_width = preview_frame_width_;
  const uint32_t frame_height = preview_frame_height_;
  const uint32_t data_size = frame_width * frame_height * bytes_per_pixel_;
